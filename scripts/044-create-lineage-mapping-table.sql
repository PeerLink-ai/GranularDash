...
-- Fixed foreign key to reference agent_id instead of id
-- Add foreign key constraint if connected_agents table exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'connected_agents') THEN
...
-- Updated sample data to use agent_id column
